...
class App extends Component {
  ...
  handleSubmit() {
    if (this.state.startDate && this.state.endDate) {
      ...
      axios({
        method: "get",
        url: `https://f8966041.ngrok.io/data/adrequests?from=${moment(
          this.state.startDate._d
        ).format("YYYY-MM-DD")}&to=${moment(this.state.endDate._d).format(
          "YYYY-MM-DD"
        )}`
      })
        .then(response => {
        ...
        .catch(err => {
          this.setState({
            showError: true,
            errorLabel: "Network unreachable",
            loading: false
          });
        });
    }
    ...
    return (
      <div style={{ margin: 10, padding: 10 }}>
        ...
        <div
          ...
          style={{ display: "inline-block", marginRight: 100 }}
        >
          <span className="toDate" style={{ marginRight: 15 }}>
            To Date
          </span>
          <div style={{ display: "inline-block", padding: 10 }}>
            <DatePicker
        ...
        <div className="submitButton" style={{ display: "inline-block" }}>
          <button
            ...
            disabled={
              this.state.startDate ? (this.state.endDate ? false : true) : true
            }
            title={
              this.state.startDate
                ? this.state.endDate ? "" : "Select both dates"
                : "Select both dates"
            }
          >
            Submit
